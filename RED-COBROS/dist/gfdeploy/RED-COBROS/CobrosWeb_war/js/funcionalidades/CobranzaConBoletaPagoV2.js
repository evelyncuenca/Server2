...
function cobranzasFormularioBoletaDePago() {
    ...
    comboIMPUESTO.addListener('select', function (esteCombo, record, index) {
        Ext.getCmp('idFechaConstitucion').focus(true, true);
    }, null, null);
                        ...
                        listeners: {
                            'specialkey': function (esteObjeto, esteEvento) {
                                if (esteEvento.getKey() === 13 || esteEvento.getKey() === 9) {
                                    ...
                                    if (true) {
                                        Ext.Msg.hide();
                                        Ext.getCmp('idNombreRazonSocialBoletaPago').setValue('Nombre Contribuyente');
                                        var conn = new Ext.data.Connection();
                                        /*conn.request({
                                         waitTitle: 'Conectando',
                                         waitMsg: 'Cargando Campos',
                    ...
                    , {
                        ...
                        listeners: {
                            'specialkey': function (esteObjeto, esteEvento) {
                                ...
                                if (esteEvento.getKey() == 13 || esteEvento.getKey() == 9) {
                                    if (!Ext.getCmp('idNumeroBoleta').disabled) {
                                        Ext.getCmp('idNumeroBoleta').focus(true, true);
                                    }
                                    else {
                        ...
                        blankText: 'Número de Boleta Obligatorio',
                        listeners: {
                            'specialkey': function (esteObjeto, esteEvento) {
                                if (esteEvento.getKey() === 13 || esteEvento.getKey() === 9) {
                                    Ext.Msg.wait('Procesando... Por Favor espere...');
                                    ...
                                    conn.request({
                                        waitTitle: 'Conectando',
                                        waitMsg: 'Cargando Deuda',
                                        url: 'cobranzas?op=CONSULTAR_DEUDA',
                                        params: {
                                            ruc: Ext.getCmp('idRucBoletaPago').getValue(),
                                            fecha_cons: Ext.getCmp('idFechaConstitucion').getValue(),
                                            nro_boleta: Ext.getCmp('idNumeroBoleta').getValue()
                                        },
                                        method: 'POST',
                                        success: function (respuestaServer) {
                                            Ext.Msg.hide();
                                            console.log(respuestaServer.responseText);
                                            var obj = Ext.util.JSON.decode(respuestaServer.responseText);
                                            if (obj.success) {

                                                console.log('aqiu ' + obj.monto);
                                                var esteObjeto = Ext.getCmp('idMonto');
                                                esteObjeto.setRawValue(obj.monto);
        ...
        buttons: [{
                ...
                handler: function (esteObjeto) {
                    ...
                    if (!aceptarTrabajando) {
                        ...
                        Ext.getCmp('idMonto').enable();
                        fp.getForm().submit({
                            method: 'POST',
                            waitTitle: 'Conectando',
                            waitMsg: 'Enviando Información...',
                            url: 'cobranzas?op=COBRO_CON_BOLETA_DE_PAGO_V2',
                            timeout: TIME_OUT_AJAX,
                            success: function (form, accion) {
                                Ext.getCmp('idMonto').disable();
