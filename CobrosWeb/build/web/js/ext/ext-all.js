...
Ext.util.Observable.observeClass=function(a){
    ...
    a.prototype.fireEvent=function(){
        return(a.fireEvent.apply(a,arguments)!==false)&&(Ext.util.Observable.prototype.fireEvent.apply(this,arguments)!==false)
        }
    };
...
Ext.apply(Ext.EventManager,function(){
    var c,j,e,b,a=Ext.lib.Dom,k=Ext.lib.Event,i=/^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/,h=0,g=0,d=Ext.isSafari?Ext.num(navigator.userAgent.toLowerCase().match(/version\/(\d+\.\d)/)[1]||2)>=3.1:!((Ext.isGecko&&!Ext.isWindows)||Ext.isOpera);
    return{
...
removeClass:function(F){
    ...
    if(G.dom.className){
        Ext.each(F,function(H){
            G.dom.className=G.dom.className.replace(b[H]=b[H]||new RegExp("(?:^|\\s+)"+H+"(?:\\s+|$)","g")," ")
            })
        }
        return G
    },
...
Ext.apply(this,b);
this.addEvents("disable","enable","beforeshow","show","beforehide","hide","beforerender","render","afterrender","beforedestroy","destroy","beforestaterestore","staterestore","beforestatesave","statesave");
this.getId();
...
insert:function(g,e){
    ...
    if(j.ownerCt==this&&this.items.indexOf(j)<g){
        ...
        if(this.fireEvent("beforeadd",this,j,g)!==false&&this.onBeforeAdd(j)!==false){
    ...
    for(g=0;g<l;g++){
        ...
        if(o.anchor){
            ...
            if(!q){
                ...
                o.anchorSpec=q={
                    right:this.parseAnchor(s[0],o.initialConfig.width,d),
                    bottom:this.parseAnchor(s[1],o.initialConfig.height,p)
                    }
                }
            e=q.right?this.adjustWidthAnchor(q.right(r),o):undefined;
        b=q.bottom?this.adjustHeightAnchor(q.bottom(j),o):undefined;
        if(e||b){
...
Ext.layout.ColumnLayout=Ext.extend(Ext.layout.ContainerLayout,{
    ...
    onLayout:function(d,j){
        ...
        var n=Ext.isIE&&j.dom!=Ext.getBody().dom?j.getStyleSize():j.getViewSize();
        if(n.width<1&&n.height<1){
            return
        }
        var l=n.width-j.getPadding("lr")-this.scrollOffset,b=n.height-j.getPadding("tb"),m=l;
        this.innerCt.setWidth(l);
        for(a=0;a<g;a++){
            k=e[a];
            if(!k.columnWidth){
                m-=(k.getSize().width+k.getEl().getMargins("lr"))
                }
            }
        m=m<0?0:m;
    for(a=0;a<g;a++){
        k=e[a];
        if(k.columnWidth){
            k.setSize(Math.floor(k.columnWidth*m)-k.getEl().getMargins("lr"))
            }
        }
    }
});
        ...
        if(!this.rendered){
            ...
            for(var A=0,B=v.length;A<B;A++){
                ...
                if(E.collapsed){
                    ...
                    E.collapsed=false;
                if(!E.rendered){
                    E.cls=E.cls?E.cls+" x-border-panel":"x-border-panel";
                    E.render(H,A)
                    }
...
getTemplateArgs:function(b){
    ...
    return{
        ...
        labelSeparator:a?"":(typeof b.labelSeparator=="undefined"?this.labelSeparator:b.labelSeparator),
...
Ext.layout.VBoxLayout=Ext.extend(Ext.layout.BoxLayout,{
    ...
    onLayout:function(e,E){
        Ext.layout.VBoxLayout.superclass.onLayout.call(this,e,E);
        var g=this.getItems(e),j,m,q,r=this.getTargetSize(E),k=r.width-E.getPadding("lr")-this.scrollOffset,y=r.height-E.getPadding("tb"),s=this.padding.left,n=this.padding.top,i=this.pack=="start",v=["stretch","stretchmax"].indexOf(this.align)==-1,B=k-(this.padding.left+this.padding.right),o=0,A=0,u=0,C=0,d=0;
        ...
        switch(this.align){
            ...
            case"center":
                ...
                var c=Math.max(0,y-this.padding.top-this.padding.bottom-C),D=c,z=[],x=[],p=0,b=Math.max(0,k-this.padding.left-this.padding.right);
        Ext.each(g,function(h){
            if(i&&h.flex){
...
Ext.each(g,function(l){
    ...
    if(this.align=="stretch"){
        l.setWidth((B-(j.left+j.right)).constrain(l.minWidth||0,l.maxWidth||1000000))
        }else{
        if(this.align=="stretchmax"){
            l.setWidth((A-(j.left+j.right)).constrain(l.minWidth||0,l.maxWidth||1000000))
            }else{
            if(this.align=="center"){
                var h=b-(l.getWidth()+j.left+j.right);
                if(h>0){
                    l.setPosition(s+j.left+(h/2),l.y)
                    }
                }
            if(i&&l.flex){
            l.setWidth(x[p++])
...
Ext.layout.HBoxLayout=Ext.extend(Ext.layout.BoxLayout,{
    ...
    onLayout:function(d,E){
        Ext.layout.HBoxLayout.superclass.onLayout.call(this,d,E);
        var e=this.getItems(d),j,c,q,r=this.getTargetSize(E),m=r.width-E.getPadding("lr")-this.scrollOffset,B=r.height-E.getPadding("tb"),u=this.padding.left,o=this.padding.top,i=this.pack=="start",z=["stretch","stretchmax"].indexOf(this.align)==-1,n=B-(this.padding.top+this.padding.bottom),C=0,s=0,v=0,b=0,g=0;
        ...
        switch(this.align){
            ...
            case"middle":
                ...
                var x=Math.max(0,m-this.padding.left-this.padding.right-b),D=x,k=[],A=[],p=0,a=Math.max(0,B-this.padding.top-this.padding.bottom);
        Ext.each(e,function(h){
            if(i&&h.flex){
...
Ext.each(e,function(t){
    var h=t.margins;
    if(this.align=="stretch"){
        t.setHeight((n-(h.top+h.bottom)).constrain(t.minHeight||0,t.maxHeight||1000000))
        }else{
        if(this.align=="stretchmax"){
            t.setHeight((s-(h.top+h.bottom)).constrain(t.minHeight||0,t.maxHeight||1000000))
            }else{
            if(this.align=="middle"){
                var l=a-(t.getHeight()+h.top+h.bottom);
                if(l>0){
                    t.setPosition(t.x,o+h.top+(l/2))
                    }
                }
            if(i&&t.flex){
            t.setHeight(A[p++])
        ...
        if(d&&!Ext.isEmpty(d.height)){
            ...
            if(b!="auto"&&b!=c){
                ...
                if(a>0&&a!=c){
                    ...
                    var g=e.getSize();
                    this.fireEvent("bodyresize",g.width,g.height)
                    }
                }
        }
}
...
var y=this.maxDate?this.maxDate.clearTime():Number.POSITIVE_INFINITY;
...
if(this.showToday){
    ...
    var c=(m<u||m>y||(G&&A&&G.test(m.dateFormat(A)))||(J&&J.indexOf(m.getDay())!=-1));
    if(!this.disabled){
...
if(c!==false){
    ...
    if(this.writer&&this.proxy.url&&!this.proxy.restful&&!Ext.data.Api.hasUniqueUrl(this.proxy,d)){
        e.xaction=d
        }
...
if(!this.convert){
    ...
    switch(this.type){
        ...
        case"int":
            b=function(g){
            return g!==undefined&&g!==null&&g!==""?parseInt(String(g).replace(e,""),10):""
            };

        break;
        case"float":
            b=function(g){
            return g!==undefined&&g!==null&&g!==""?parseFloat(String(g).replace(e,""),10):""
            };
...
Ext.data.DataReader.prototype={
    ...
    realize:function(b,d){
        if(Ext.isArray(b)){
            for(var c=b.length-1;c>=0;c--){
                if(Ext.isArray(d)){
                    this.realize(b.splice(c,1).shift(),d.splice(c,1).shift())
                    }else{
    ...
    var a=this.extractValues(d,b.fields.items,b.fields.items.length);
    b.phantom=false;
    b._phid=b.id;
...
isData:function(a){
    return(a&&Ext.isObject(a)&&!Ext.isEmpty(a[this.meta.idProperty]))?true:false
    }
};
...
toHash:function(e){
    var d=e.fields.map,c={},b=(this.writeAllFields===false&&e.phantom===false)?e.getChanges():e.data,a;
...
buildUrl:function(d,b){
    ...
    if(a){
        ...
        if((this.prettyUrls===true||this.restful===true)&&b instanceof Ext.data.Record&&!b.phantom){
        c+="/"+b.id
        }
        if(e){
        c+=e
...
doRequest:function(g,d,i,c,b,e,a){
    ...
    if(this.restful===true&&d instanceof Ext.data.Record&&!d.phantom){
        this.conn.url+="/"+d.id
        }
    }
if(this.useAjax){
...
onWrite:function(g,i,c,b){
    ...
    if(d[a.meta.successProperty]===false){
        ...
        this.fireEvent("write",this,g,d[a.meta.root],d,b,i.request.arg)
        }
        i.request.callback.call(i.request.scope,d[a.meta.root],d,d[a.meta.successProperty])
    },
destroy:function(){
    if(!this.useAjax){
        this.conn.abort()
...
readRecords:function(a){
    ...
    if(a.metaData){
        ...
        this.recordType=Ext.data.Record.create(a.metaData.fields);
...
for(var e=0;e<k;e++){
    var b=l[e];
    var g=new m(this.extractValues(b,t,p),this.getId(b));
    g.json=b;
    d[e]=g
    }
    return{
...
buildExtractors:function(){
    ...
    if(l.totalProperty){
        this.getTotal=this.getJsonAccessor(l.totalProperty)
        }
        if(l.successProperty){
        this.getSuccess=this.getJsonAccessor(l.successProperty)
        }
        this.getRoot=l.root?this.getJsonAccessor(l.root):function(g){
        return g
        };
...
readResponse:function(b,a){
    var c=(a.responseText!==undefined)?Ext.decode(a.responseText):a;
    if(!c){
...
Ext.data.ArrayReader=Ext.extend(Ext.data.JsonReader,{
    readRecords:function(q){
        ...
        if(!this.getRoot){
            this.getRoot=h.root?this.getJsonAccessor(h.root):function(i){
                return i
                };
    ...
    for(var x=0;x<t.length;x++){
        ...
        var m=((d||d===0)&&r[d]!==undefined&&r[d]!==""?r[d]:null);
        for(var w=0,l=p.length;w<l;w++){
            var z=p.items[w];
            var u=z.mapping!==undefined&&z.mapping!==null?z.mapping:w;
...
readRecords:function(z){
    ...
    for(var u=0,x=A.length;u<x;u++){
        ...
        for(var t=0,k=p.length;t<k;t++){
            var y=p.items[t];
            var g=l.selectValue(Ext.value(y.mapping,y.name,true),r,y.defaultValue);
            g=y.convert(g,r);
            a[y.name]=g
            }
            var c=new b(a,m);
...
applySort:function(){
    ...
    if(!this.groupOnSort&&!this.remoteGroup){
        ...
        if(a&&(a!=this.sortInfo.field||this.groupDir!=this.sortInfo.direction)){
...
Ext.data.DirectStore=function(a){
    ...
    Ext.data.DirectStore.superclass.constructor.call(this,Ext.apply(a,{
        proxy:(typeof(a.proxy)=="undefined")?new Ext.data.DirectProxy(Ext.copyTo({},a,"paramOrder,paramsAsHash,directFn,api")):a.proxy,
        reader:(typeof(a.reader)=="undefined"&&typeof(a.fields)=="object")?new Ext.data.JsonReader(Ext.copyTo({},a,"totalProperty,root,idProperty"),a.fields):a.reader
        }))
    };
...
doSend:function(d){
    ...
    e[typeof this.enableUrlEncode=="string"?this.enableUrlEncode:"data"]=Ext.encode(b);
    g.params=e
    }else{
    g.jsonData=b
    }
...
for(var j=0,l=o.length;j<l;j++){
    if(o[j]&&c[o[j]]){
        var n=c[o[j]];
        this[n]=new Ext.Resizable.Handle(this,n,this.disableTrackOver,this.transparent)
        }
    }
...
var j=function(v){
    ...
    for(var w in p){
        if(typeof p[w]!="function"){
            if(v[w]){
                p[w].show();
                p[w].setText(typeof v[w]=="string"?v[w]:Ext.MessageBox.buttonText[w]);
...
updateText:function(z){
    ...
    if(Ext.isIE&&x>0){
        ...
        var v=Math.max(Math.min(b.width||x+B+y+A,this.maxWidth),Math.max(b.minWidth||this.minWidth,m||0));
    if(b.prompt===true){
        t.setWidth(v-x-y-A)
        }
...
Ext.ListView=Ext.extend(Ext.DataView,{
    ...
    initComponent:function(){
        if(this.columnResize){
            ...
            if(!this.internalTpl){
            this.internalTpl=new Ext.XTemplate('<div class="x-list-header"><div class="x-list-header-inner">','<tpl for="columns">','<div style="width:{width}%;text-align:{align};"><em unselectable="on" id="',this.id,'-xlhd-{#}">',"{header}","</em></div>","</tpl>",'<div class="x-clear"></div>',"</div></div>",'<div class="x-list-body"><div class="x-list-body-inner">',"</div></div>")
            }
            if(!this.tpl){
            this.tpl=new Ext.XTemplate('<tpl for="rows">',"<dl>",'<tpl for="parent.columns">','<dt style="width:{width}%;text-align:{align};"><em unselectable="on">',"{[values.tpl.apply(parent)]}","</em></dt>","</tpl>",'<div class="x-clear"></div>',"</dl>","</tpl>")
            }
...
onItemIconChanged:function(d,a,c){
    ...
    if(b){
        Ext.fly(b).child("span.x-tab-strip-text").replaceClass(c,a)
        }
    },
getTabEl:function(a){
    return document.getElementById(this.id+this.idDelimiter+this.getComponent(a).getItemId())
    },
onResize:function(){
...
autoSizeTabs:function(){
    ...
    for(var e=0,j=m.length;e<j;e++){
        var l=m[e];
        var n=Ext.fly(l).child(".x-tab-strip-inner",true);
        var g=l.offsetWidth;
        var d=n.offsetWidth;
...
getTemplateArgs:function(){
    var a=(this.cls||"");
    a+=(this.iconCls||this.icon)?(this.text?" x-btn-text-icon":" x-btn-icon"):" x-btn-noicon";
    if(this.pressed){
        a+=" x-btn-pressed"
        }
        return[this.text||"&#160;",this.type,this.iconCls||"",a,"x-btn-"+this.scale+" x-btn-icon-"+this.scale+"-"+this.iconAlign,this.getMenuClass()]
    },
...
onRender:function(c,a){
    if(!this.template){
        if(!Ext.Button.buttonTemplate){
            Ext.Button.buttonTemplate=new Ext.Template('<table cellspacing="0" class="x-btn {3}"><tbody class="{4}">','<tr><td class="x-btn-tl"><i>&#160;</i></td><td class="x-btn-tc"></td><td class="x-btn-tr"><i>&#160;</i></td></tr>','<tr><td class="x-btn-ml"><i>&#160;</i></td><td class="x-btn-mc"><em class="{5}" unselectable="on"><button class="x-btn-text {2}" type="{1}">{0}</button></em></td><td class="x-btn-mr"><i>&#160;</i></td></tr>','<tr><td class="x-btn-bl"><i>&#160;</i></td><td class="x-btn-bc"></td><td class="x-btn-br"><i>&#160;</i></td></tr>',"</tbody></table>");
...
isClickOnArrow:function(a){
    return this.arrowAlign!="bottom"?a.getPageX()>this.el.child(this.buttonSelector).getRegion().right:a.getPageY()>this.el.child(this.buttonSelector).getRegion().bottom
    },
...
Ext.layout.ToolbarLayout=Ext.extend(Ext.layout.ContainerLayout,{
    ...
    onLayout:function(d,g){
        if(!this.leftTr){
            ...
            g.insertHtml("beforeEnd",'<table cellspacing="0" class="x-toolbar-ct"><tbody><tr><td class="x-toolbar-left" align="left"><table cellspacing="0"><tbody><tr class="x-toolbar-left-row"></tr></tbody></table></td><td class="x-toolbar-right" align="right"><table cellspacing="0" class="x-toolbar-right-ct"><tbody><tr><td><table cellspacing="0"><tbody><tr class="x-toolbar-right-row"></tr></tbody></table></td><td><table cellspacing="0"><tbody><tr class="x-toolbar-extras-row"></tr></tbody></table></td></tr></tbody></table></td></tr></tbody></table>');
...
getItemWidth:function(a){
    return a.hidden?(a.xtbWidth||0):a.getDomPositionEl().dom.parentNode.offsetWidth
    },
...
beforeMoreShow:function(b){
    var g=this.container.items.items,a=g.length,k,j,d=function(h,c){
        return h.isXType("buttongroup")&&!(c instanceof Ext.Toolbar.Separator)
        };
    ...
    if(b.isFormField||b.render){
        ...
        if(b.tag){
            ...
            if(b.tagName){
                ...
                if(Ext.isObject(b)){
                    b=b.xtype?this.constructItem(b):this.constructButton(b)
                    }
                }
        }
}
...
constructButton:function(d){
    var c=d.events?d:this.constructItem(d,d.split?"splitbutton":this.defaultType);
...
Ext.History=(function(){
    ...
    function g(){
        ...
        function i(m){
        var l=['<html><body><div id="state">',m,"</div></body></html>"].join("");
        try{
...
getTargetXY:function(){
    if(this.anchor){
        ...
        var j=(this.anchorToTarget&&!this.trackMouse)?this.el.getAlignToXY(this.anchorTarget,this.getAnchorAlign()):this.targetXY;
        var a=Ext.lib.Dom.getViewWidth()-5;
        var g=Ext.lib.Dom.getViewHeight()-5;
        var i=(document.documentElement.scrollLeft||document.body.scrollLeft||0)+5;
        var h=(document.documentElement.scrollTop||document.body.scrollTop||0)+5;
...
initComponent:function(){
    ...
    if(!this.url){
        ...
        this.addEvents("itemmouseover","itemmouseout","itemclick","itemdoubleclick","itemdragstart","itemdrag","itemdragend");
    this.store=Ext.StoreMgr.lookup(this.store)
    },
...
constrainScroll:function(c){
    ...
    if(this.floating){
        ...
        if(b>a&&a>0){
        this.activeMax=a-this.scrollerHeight*2-this.el.getFrameWidth("tb")-Ext.num(this.el.shadowOffset,0);
...
adjustWidth:function(a,b){
    if(typeof b=="number"&&(Ext.isIE&&(Ext.isIE6||!Ext.isStrict))&&/input|textarea/i.test(a)&&!this.inEditor){
        return b-3
        }
        return b
    }
...
filterKeys:function(a){
    ...
    if(!this.maskRe.test(String.fromCharCode(a.getCharCode()))){
        a.stopEvent()
        }
    },
setValue:function(a){
...
fireKey:function(b){
    var a=function(c){
        if(c.isNavKeyPress()&&!this.isExpanded()&&!this.delayedCheck){
            this.fireEvent("specialkey",this,c)
            }
        };

if(this.inEditor&&Ext.isWebKit&&b.getKey()==b.TAB){
...
onKeyUp:function(b){
    var a=b.getKey();
    if(this.editable!==false&&(a==b.BACKSPACE||!b.isSpecialKey())){
        this.lastKey=a;
        ...
        b.add("-",{
            ...
            menu:new Ext.menu.ColorMenu({
                ...
                listeners:{
                    scope:this,
                    select:function(g,e){
                        this.execCmd("forecolor",Ext.isWebKit||Ext.isIE?"#"+e:e);
                        this.deferFocus()
                        }
                    },
            clickEvent:"mousedown"
                ...
                listeners:{
                    ...
                    select:function(g,e){
                        if(Ext.isGecko){
                            ...
                            this.execCmd(Ext.isOpera?"hilitecolor":"backcolor",Ext.isWebKit||Ext.isIE?"#"+e:e);
                            this.deferFocus()
                            }
                        }
                },
...
onResize:function(b,c){
    ...
    if(this.el&&this.iframe){
        if(typeof b=="number"){
            ...
            if(this.doc){
                this.getEditorBody().style.height=Math.max((a-(this.iframePad*2)),0)+"px"
                }
            }
    }
},
...
pushValue:function(){
    if(this.initialized){
        ...
        if(!this.activated&&a.length<1){
            ...
            if(Ext.isGecko){
                var c=this.doc,b=c.designMode.toLowerCase();
...
Ext.form.TimeField=Ext.extend(Ext.form.ComboBox,{
    ...
    initComponent:function(){
        if(typeof this.minValue=="string"){
            ...
            if(!this.store){
            var b=this.parseDate(this.minValue)||new Date(this.initDate).clearTime();
...
if(h.sort&&this.store){
    this.store[this.store.remoteSort?"setDefaultSort":"sort"](h.sort.field,h.sort.direction)
    }
    delete h.columns;
delete h.sort;
    ...
    for(var v=0,y=k.length;v<y;v++){
        ...
        for(var x=0;x<q;x++){
            ...
            if(Ext.isEmpty(u.value)){
                ...
                if(this.markDirty&&o.dirty&&typeof o.modified[A.name]!=="undefined"){
...
renderUI:function(){
    ...
    if(c.enableColumnResize!==false){
        this.splitZone=new Ext.grid.GridView.SplitDragZone(c,this.mainHd.dom)
        }
...
Ext.destroy(this.resizeMarker,this.resizeProxy,this.focusEl,this.mainBody,this.scroller,this.mainHd,this.mainWrap,this.dragZone,this.splitZone,this.columnDrag,this.columnDrop);
...
handleHdMove:function(h,d){
    if(this.activeHd&&!this.headersDisabled){
        ...
        if(a-g.left<=b&&this.cm.isResizable(this.activeHdIndex-1)){
            c.cursor=Ext.isAir?"move":Ext.isWebKit?"e-resize":"col-resize"
            }else{
            if(g.right-a<=(!this.activeHdBtn?b:2)&&this.cm.isResizable(this.activeHdIndex)){
                c.cursor=Ext.isAir?"move":Ext.isWebKit?"w-resize":"col-resize"
                }else{
                c.cursor=""
                }
            }
...
onNodeDrop:function(b,m,g,c){
    ...
    if(d!=b){
        ...
        var o=(a.right-j)<=((a.right-a.left)/2)?"after":"before";
...
onEditorKey:function(i,h){
    ...
    if(c==h.ENTER){
        ...
        if(this.moveEditorOnEnter!==false){
            if(a){
                j=d.walkCells(b.row-1,b.col,-1,this.acceptsNav,this)
                }else{
                j=d.walkCells(b.row+1,b.col,1,this.acceptsNav,this)
                }
            }
...
Ext.grid.TemplateColumn=Ext.extend(Ext.grid.Column,{
    constructor:function(a){
        ...
        var b=typeof Ext.isObject(this.tpl)?this.tpl:new Ext.XTemplate(this.tpl);
    ...
    if(this.colModel.isCellEditable(b,h)){
        ...
        if(this.fireEvent("beforeedit",d)!==false&&!d.cancel){
            ...
            if(!a.rendered){
                ...
                a.startEdit(this.view.getCell(h,b).firstChild,e===undefined?"":e)
                }).defer(50,this)
            }
        }
},
...
doRender:function(d,j,t,a,s,v){
    if(j.length<1){
        ...
        var C=this.getGroupField(),q=this.cm.findColumnIndex(C),z;
    ...
    for(x=0,y=j.length;x<y;x++){
        ...
        if(!n||n.group!=z){
            p=u+"-gp-"+C+"-"+Ext.util.Format.htmlEncode(z);
            var c=typeof this.state[p]!=="undefined"?!this.state[p]:this.startCollapsed;
    ...
    for(x=0,y=B.length;x<y;x++){
        ...
        A[A.length]=Ext.grid.GroupingView.superclass.doRender.call(this,d,z.rs,t,z.startRow,s,v);
